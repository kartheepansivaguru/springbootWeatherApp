         ...
         if (!jsonStrings.isEmpty()) {
             jsonStrings.forEach(jsonString -> {
                 ...
                 try {
                    ...
                    Weather weather = new Weather(id, this.getDate(dateString),  location, temperature);
...
private Map<Date,List<Weather>> getWeatherMapBasedOnDate(List<Weather> dtoList){
	Map<Date, List<Weather>> map = dtoList.stream().collect(Collectors.groupingBy(Weather::getDateRecorded));
	return map;
}
